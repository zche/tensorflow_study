...
# 下面吧训练集中的手写黑白字体变成标准的四维张量形式，即（样本数量，长，宽，1）
# 并把像素值变成浮点格式

X_train = X_train.reshape(X_train.shape[0],28,28,1).astype('float32')
X_test = X_test.reshape(X_test.shape[0],28,28,1).astype('float32')

# 由于每个像素值都介于0到255之间，所以这里统一除以255，吧像素值控制在0到1之间
X_train /= 255
X_test /=255 
...
# 搭建卷积神经网络
model = Sequential()
# 添加一层卷积层，构造64个过滤器，每个过滤器覆盖范围是3*3
...
# 放入批量脚本，进行训练
model.fit(X_train,y_train_one,validation_data=(X_test,y_test_one),epochs=20,batch_size=128)
# 在测试集上评价模型的准确率
