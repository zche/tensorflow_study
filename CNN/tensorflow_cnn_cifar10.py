...
# 使用的数据集是CIFAR-10，这是一个经典的数据集，包含60000张32*32的彩色图像，其中训练集50000个，测试集10000张
# 一共标注为10类，每一类图片6000张。10类分别是airplane,automobile,bird,cat,deer,dog,frog,horse,ship,truck
# 我们载入一些常用库，比如Numpy 和 time ,并载入TensorFlow Models中自动下载、读取Cifar-10数据的类
max_steps = 30000
batch_size = 128

def variable_with_weight_losses(shape,stddev,wl):
    # 定义初始化weights的函数，和之前一样依然使用tf.truncated_normal截断的正太分布来初始化
...
# 使用cifar10_input类中的distorted_inputs 函数产生训练需要使用的数据，包括特征及其对应的label,这里是封装好的tensor.
# 每次执行都会生成一个batch_size 的数量的样本。需要注意的是这里对数据进行了Data Augmentation(数据增强)
...
# 生成测试数据，不过这里不需要进行太多处理，不需要对图片进行翻转或修改亮度、对比度，不过需要裁剪图片正中间的24*24大小的区域
# 并进行数据标准化操作
...
# 使用尺寸3*3步长2*2的最大池化层处理数据，这里最大池化的尺寸和步长不一样，可以增加数据的丰富
...
# Alex 在ImageNet数据集上的实验表明，使用LRN后CNN在Top1的错误率可以降低1.4%，因此其在经典AlexNet中使用了LRN层
...
# 还有这里的bias值全部初始化为0.1，而不是0.最好，调换了最大池化层和LRN层的顺序，先进性LRN层
...
# 两个卷积层之后，是全连接层
# 先把第二个卷积层之后的输出结果flatten，使用tf.reshape函数将每个样本都变成一维向量，使用
reshape = tf.reshape(pool2,[batch_size,-1])
# dim 就是上面-1 即多少列
dim = reshape.get_shape()[1].value
# 接着初始化权值，隐含节点384个，正太分布的标准差设为0.04，bias的值也初始化为0.1
# 注意这里我们希望这个全连接层不要过拟合，因此设了一个非零的weight loss值为0.04，让这一层具有
# 384是根据AlexNet 的设置的，(可以随便写)，指的是隐藏节点是384个
weight3 = variable_with_weight_losses(shape=[dim,384],stddev=0.04,wl=0.004)
bias3 = tf.Variable(tf.constant(0.1,shape=[384]))
# 最后我们依然使用Relu计划函数进行非线性化
...
# 这里没有用之前的softmax输出最后结果，这里把softmax操作放在了计算loss部分，其实我们不需要
# 处理就可以获得最终分类结果（直接比较inference输出的各类的数值大小即可），计算softmax只要
...
# 网络结构，是需要大量实际摸索的
# 完成模型inference的构建，接下来就是计算CNN的loss，这里依然是用cross_entropy，这里我们把
# 合在了一起，即tf.nn.sparse_softmax_cross_entropy_with_logits()
# 这里使用tf.reduce_mean() 对cross entropy计算均值，再使用tf.add_to_collection()把
# losses的collection中，最后，使用tf.add_n将整体losses的collection集合中的全部loss求和，
...
def loss(logits,labels):
    ...
    lossList = tf.get_collection('losses')
    return tf.add_n(lossList)

loss = loss(logits=logits,labels=label_holder)
# 优化器依然选择Adam Optimizer，学习速率0.001
train_op = tf.train.AdamOptimizer(1e-3).minimize(loss)

# 使用tf.nn.in_top_k()函数求输出结果中top k的准确率，默认使用top 1,也就是输出分数最高的
top_k_op = tf.nn.in_top_k(logits,label_holder,1)

sess = tf.InteractiveSession()
tf.global_variables_initializer().run()
# 前面对图像进行数据增强的操作需要消耗大量CPU时间，因此distorted_inputs使用了16个独立的线程
# 在需要使用时会通过TensorFlow queue进行调度
# 启动图片数据增强的线程队列，这里一共使用了16个线程来进行加速，如果不启动线程，name或许
...
# 一共跑了大概1小时40分左右，正确率为83.3%
sess.close()

# 最终，在cifar-10数据集上，通过一个短时间小迭代的训练，可以达到大致73%的准确率，持续增加到max_steps,可以期望准确率逐渐增加
# 如果max_steps 比较大，则推荐使用学习速率衰减decay的SGD进行训练（Adam计算量比较大！），这样训练过程中能达到的准确率峰值会比较高，大致有86%
# 其中L2正则以及LRN层的使用都对模型准确率有提升作用，他们都可以提升模型的泛化能力

# 数据增强Data Augmentation 在我们的训练中作用很大，它可以给单幅图增加多个副本，提高图片的利用率，防止对某一张图片结构的学习过拟合
# 这刚好是利用了图片本身的性质，图片的冗余信息量比较大，因此可以制造不同的噪声并让图片依然可以被识别出来。如果神经网络可以克服这些
