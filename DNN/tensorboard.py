...
with tf.name_scope('input_reshape'):
    # 784维度变形为图片保持到节点
    # -1 代表进来的图片的数量(-1代表不知道)、28，28是图片的高和宽，1是图片的颜色通道
...
# 使用tf.summary.histogram 直接记录变量var的直方图数据
...
def nn_layer(input_tensor,input_dim,output_dim,layer_name,act=tf.nn.relu):
    with tf.name_scope(layer_name):
        ...
        return activations
# 我们使用刚刚定义的函数创建一层神经网络，输入维度是图片的尺寸 784=28*28
...
# 同时，将Session计算图sess.graph 加入训练过程，这样在TensorBoard的Graphs窗口中就能展示
# 整个计算图的可视化效果，最后初始化全部变量
tf.global_variables_initializer().run()

# 定义feed_dict，如果是训练，需要设置dropout，如果是测试，keep_prob设置为1
def feed_dict(train):
    if train:
